...
async function handle(request,env){
  ...
  if(p==="/api/speedmap/races"&&request.method==="GET"){
    try{
      ...
      const d=await tabJSON(`${TAB_BASE}/meetings?date=${encodeURIComponent(date)}&enc=json`);
      const list=d?.data||d?.meetings||d||[];
      const meetings=(Array.isArray(list)?list:list.meetings||[]).map(m=>({
        id:m.meeting_id??m.id, venue:m.venue_name||m.venue||m.track||"",
        meeting_number:meetingNumberOf(m), country:m.country||m.country_code||""
      })).filter(m=>m.id&&m.venue);
      const races=[];
      for(const m of meetings){
        try{
          const md=await tabJSON(`${TAB_BASE}/meetings/${encodeURIComponent(m.id)}?enc=json`);
          const rr=md?.races||md?.data?.races||md?.data||[];
          for(const r of (Array.isArray(rr)?rr:[])){
            const type=String(r.race_type||r.category||r.code||"thoroughbred").toLowerCase();
            if(type && !type.includes("thorough") && !type.includes("gallop") && type!=="tb") continue;
            races.push({event_id:r.event_id??r.id,meeting_id:m.id,venue:m.venue,name:r.description||r.race_name||r.name||"",number:r.race_number??r.number??"",distance:r.distance??"",meeting_number:meetingNumberOf(r)||m.meeting_number,country:m.country});
          }
        }catch{}
      }
      races.sort((a,b)=>(Number(a.meeting_number)||999)-(Number(b.meeting_number)||999)||Number(a.number)-Number(b.number));
      return json({date,meetings,races});
