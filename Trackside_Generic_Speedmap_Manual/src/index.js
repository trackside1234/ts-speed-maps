...
function buildElement(env,race,rows,index){
  ...
  const t=titleParts(race);
  const nested=rows.map(buildRunner).join("");
  const payload=`<payload xmlns="http://www.vizrt.com/types"><field name="00"><value>${esc(t.heading)}</value></field><field name="01"><value>${esc(t.secondary)}</value></field><field name="02"><value>${esc(t.extra)}</value></field><field name="106"><value>${tpl.state}</value></field><field name="104"><list /></field><field name="105"><value>${rows.length}</value></field></payload>`;
  const desc=`${t.heading}/SPEED MAP/${rows.length}/${tpl.state}`;
...
<entry name="data">
<entry name="00">${esc(t.heading)}</entry>
<entry name="01">${esc(t.secondary)}</entry>
<entry name="02">${esc(t.extra)}</entry>
...
<entry name="settings"><entry name="tabfields"><entry name="00"/><entry name="01"/><entry name="02"/><entry name="104"/><entry name="105"/><entry name="106"/></entry><entry name="isfilescript">false</entry><entry name="modified">${new Date().toISOString().slice(0,19)}</entry></entry>
...
async function handle(request,env){
  ...
  if(p==="/api/speedmap"&&request.method==="POST"){
    try{
      ...
      await env.SELECTIONS_KV.put(speedmapKey(eventId),JSON.stringify({eventId,runners,updatedAt:new Date().toISOString()}));
  ...
  if(p==="/api/export/speedmaps"&&request.method==="GET"){
    try{
      ...
      for(let i=0;i<(show.races||[]).length;i++){
        ...
        if(!map){const ev=await eventById(race.event_id);map={runners:(ev.runners||[]).filter(r=>!r.is_scratched).sort((a,b)=>Number(a.barrier??9999)-Number(b.barrier??9999)).map(r=>({barrier:r.barrier,runnerNumber:String(r.runner_number??""),horseName:r.name||"",speedMap:50}))};}
        const ev=await eventById(race.event_id);
        const mergedRace={...race,venue:ev.race.venue||race.venue_name,name:ev.race.name||race.description,number:ev.race.number||race.race_number,distance:ev.race.distance||race.distance,meeting_number:ev.race.meeting_number||race.meeting_number};
