...
const GENERIC_RACES_KEY = "generic-speedmap:races";
...
async function getGenericRaces(env){
  const raw=await env.SELECTIONS_KV.get(GENERIC_RACES_KEY);
  return raw?JSON.parse(raw):{races:[]};
}
...
async function loadMaps(env){
  const selection=await getGenericRaces(env);
  const maps=[];
  for(const race of (selection.races||[])){
  ...
  return {races:selection.races||[],maps};
}
async function handle(request,env){
  ...
  if(p==="/api/speedmap/races"&&request.method==="GET"){
    try{
      const date=u.searchParams.get("date")||"";
      if(!date)return json({error:"Date required"},400);
      // Use the same TAB Affiliates API pattern as the working Form/Review tools:
      // meetings are returned with their races embedded in data.meetings[].races[].
      const q=`${TAB_BASE}/meetings?category=T&date_from=${encodeURIComponent(date)}&date_to=${encodeURIComponent(date)}&enc=json`;
      const d=await tabJSON(q);
      const meetings=Array.isArray(d?.data?.meetings)?d.data.meetings:[];
      if(!meetings.length)return json({date,meetings:[],races:[],message:"No thoroughbred TAB meetings were returned for this date."});
      const outMeetings=meetings.map((m,i)=>({
        id:m.id??m.meeting_id??m.mnemonic??`meeting-${i}`,
        venue:m.name||m.venue_name||m.venue||m.track||"",
        meeting_number:meetingNumberOf(m),
        country:m.country||m.country_code||"",
        category:m.category||m.category_name||"Thoroughbred"
      })).filter(m=>m.id&&m.venue);
      const races=[];
      meetings.forEach((m,mi)=>{
        const venue=m.name||m.venue_name||m.venue||m.track||"";
        const meetingId=m.id??m.meeting_id??m.mnemonic??`meeting-${mi}`;
        const meetingNumber=meetingNumberOf(m);
        for(const r of (Array.isArray(m.races)?m.races:[])){
          const eventId=r.id??r.event_id;
          if(!eventId)continue;
          races.push({
            event_id:String(eventId),
            meeting_id:String(meetingId),
            venue,
            name:r.name||r.description||r.race_name||`RACE ${r.number??r.race_number??""}`,
            number:r.number??r.race_number??"",
            distance:r.distance??r.distance_meters??"",
            meeting_number:meetingNumberOf(r)||meetingNumber,
            country:r.country||r.country_code||m.country||m.country_code||""
          });
        }
      });
      races.sort((a,b)=>(Number(a.meeting_number)||9999)-(Number(b.meeting_number)||9999)||Number(a.number)-Number(b.number));
      return json({date,meetings:outMeetings,races});
    }catch(e){return json({error:e.message},502)}
  }
  if(p==="/api/speedmap/races"&&request.method==="POST"){
    try{
      const body=await request.json();
      const races=Array.isArray(body.races)?body.races.map(r=>({event_id:String(r.event_id),venue:r.venue||"",name:r.name||"",number:r.number??"",distance:r.distance??"",meeting_number:r.meeting_number??"",country:r.country||""})).filter(r=>r.event_id):[];
      await env.SELECTIONS_KV.put(GENERIC_RACES_KEY,JSON.stringify({date:String(body.date||""),races,updatedAt:new Date().toISOString()}));
  ...
  if(p==="/api/export/speedmaps"&&request.method==="GET"){
    try{
      const selection=await getGenericRaces(env); if(!selection.races?.length)return json({error:"No races have been selected yet."},404);
      const elements=[];
      for(let i=0;i<selection.races.length;i++){
        const race=selection.races[i]; const raw=await env.SELECTIONS_KV.get(speedmapKey(String(race.event_id)));
      ...
      return new Response(xml,{headers:{"content-type":"application/xml; charset=utf-8","content-disposition":`attachment; filename="generic-speedmaps-${selection.date||"export"}.xml"`}});
    }catch(e){return json({error:e.message},400)}
  }
